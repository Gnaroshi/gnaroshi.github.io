...
export type ImplementationEntry = { id: string; data: { locale: Locale; title: string; date: Date; status: ImplementationStatus; relatedPapers: string[]; relatedProjects: string[]; repoUrl?: string; demoUrl?: string; goal: string; result?: string; failureReason?: string; lessons: string[]; tags: string[]; visibility: "public" | "unlisted" } };
type FeedImplementation = { id: string; schemaVersion: number; visibility: "public" | "unlisted"; paperIds: string[]; projectIds: string[]; date: string; status: ImplementationStatus; title: string; goal: string; result?: string; failureReason?: string; lessons?: string[]; tags: string[]; repoUrl?: string; demoUrl?: string };

export const implementationStatusLabels: Record<ImplementationStatus, string> = { planned: "Planned", "in-progress": "In progress", reproduced: "Reproduced", "partially-reproduced": "Partially reproduced", failed: "Failed", abandoned: "Abandoned", shipped: "Shipped" };
export async function getAllImplementationAttempts(locale: Locale = "en"): Promise<ImplementationEntry[]> {
  ...
  return sortImplementationAttempts(records.map((item) => ({ id: item.id, data: { locale, title: item.title, date: new Date(`${item.date}T00:00:00.000Z`), status: item.status, relatedPapers: item.paperIds, relatedProjects: item.projectIds, ...(item.repoUrl ? { repoUrl: item.repoUrl } : {}), ...(item.demoUrl ? { demoUrl: item.demoUrl } : {}), goal: item.goal, ...(item.result ? { result: item.result } : {}), ...(item.failureReason ? { failureReason: item.failureReason } : {}), lessons: item.lessons ?? [], tags: item.tags, visibility: item.visibility } })));
}
